...
class RegistrationForm(forms.Form):
    ...
    phone = forms.CharField(widget=forms.TextInput(attrs=attrs_dict),
                             label=_(u'phone number'), required=False)
    ...
    def clean_phone(self):
        """Varify that the pnone number in right form."""

        if self.cleaned_data['phone']:
            if not re.match(r"[-+., 0-9]+", self.cleaned_data['phone']):
                raise forms.ValidationError(
                            _(u'Phone number can only contain digits, spaces or ' \
                                'symbols "-, +, .,"'))
        return self.cleaned_data['phone']

    def save(self, profile_callback=None):
        """
        Create the new ``User``, ``UserProfile`` and ``RegistrationProfile``, 
        ...
        if self.cleaned_data['phone']:
                user_profile = new_user.userprofile_set.create(phone_number=\
...
class RegistrationFormUniqueEmail(RegistrationForm):
    ...
    def clean_email(self):
        ...
        """
        if User.objects.filter(email__iexact=self.cleaned_data['email']): #TODO: Why should it be unique?
